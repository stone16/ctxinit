...
`;
  ...
  /**
   * Get the best "first paragraph" for a rule summary (skipping headings/preamble).
   */
  protected getRuleSummaryParagraph(rule: ParsedRule): string {
    const paragraphs = rule.content
      .split(/\n\s*\n/)
      .map((p) => p.trim())
      .filter(Boolean);

    const isSkippable = (paragraph: string): boolean => {
      // Skip headings and low-signal preambles so AGENTS.md summaries contain actionable text.
    ...
    let summary =
      paragraphs.find((p) => !isSkippable(p)) ||
      paragraphs[0] ||
      '';
    ...
    if (summary) {
      ...
      if (summary.length > 800) {
        summary = `${summary.slice(0, 800).trimEnd()}...`;
      }
    }
  ...
  /**
   * Get summary of rule (description + summary paragraph)
   */
  protected getRuleSummary(rule: ParsedRule): string {
    ...
    return [desc, summary].filter(Boolean).join('\n\n');
  }
