...
export async function runVerify(options: VerifyCommandOptions): Promise<number> {
  ...
  if (options.json) {
    ...
    for (const result of results) {
      ...
      if (!result.exists) {
        if (options.verbose) {
          console.log(chalk.gray(`  ⏭️  ${relativePath} - not found (skipped)`));
        }
        continue;
      }

      if (!result.hasChecksum) {
        console.log(chalk.yellow(`  ⚠️  ${relativePath} - no checksum found`));
        continue;
      }
      ...
      if (result.valid) {
        ...
        console.log(chalk.red(`  ❌ ${relativePath} - MODIFIED`));
        if (options.verbose) {
    ...
    console.log('');
    if (filesVerified === 0) {
      console.log(chalk.yellow('⚠️  No verifiable files found.'));
      console.log(chalk.gray('   Run `ctx build` first to generate compiled outputs.'));
    } else if (allValid) {
      console.log(chalk.green(`✅ All ${filesVerified} file(s) verified successfully`));
    } else {
      console.log(chalk.red(`❌ Verification failed - some files have been modified`));
